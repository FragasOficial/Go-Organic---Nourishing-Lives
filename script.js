        ...
        // 1. Primeiro declare a função renderChatMessages
function renderChatMessages(productId) {
    const chatContainer = document.getElementById('chatMessages');
    chatContainer.innerHTML = '';
    
    chatMessages[productId].forEach(msg => {
        const messageDiv = document.createElement('div');
        messageDiv.className = `message ${msg.sender === 'seller' ? 'received' : 'sent'}`;
...
// 2. Depois declare a função openChat que usa renderChatMessages
function openChat(e) {
    e.preventDefault();
    ...
    if (!chatMessages[productId]) {
        chatMessages[productId] = [
            {
                text: `Olá! Sou ${seller}. Como posso ajudar você com o produto ${product.name}?`,
                sender: 'seller',
    ...
    renderChatMessages(productId); // Agora esta função já está definida
    ...
    document.getElementById('chatModal').style.display = 'block';
        ...
        // Sistema de Chat - Variáveis globais
    ...
    if (message && currentChatProduct) {
        ...
        setTimeout(() => {
            ...
            if (chatBtn) {
                ...
                setTimeout(() => chatBtn.classList.remove('new-message'), 2000);
            }
...
document.querySelector('#chatModal .close').addEventListener('click', () => {
    document.getElementById('chatModal').style.display = 'none';
});

// Atualize os listeners dos botões de chat existentes
document.querySelectorAll('.chat-btn').forEach(btn => {
    btn.addEventListener('click', openChat);
});

// Adicione esta função para inicializar o chat quando a página carrega
...
// Chame esta função no final do seu script
initChatSystem();
